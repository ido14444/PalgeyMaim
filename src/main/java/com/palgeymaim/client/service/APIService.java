...
public class APIService {
	...
	public Optional<List<IdModel>> getAllUnits() {
		...
		return extractListOfItems(unitsInfo).map(
				e -> e.stream().map(item -> new IdModel(item.getID(), item.getName())).collect(Collectors.toList()));
	}
	...
	public Optional<List<IdModel>> getControllersOfUnit(String unitId) {
		...
		return extractListOfItems(controllers).map(e -> e.stream().filter(item -> item.getID().startsWith("Controller"))
				.map(item -> new IdModel(item.getID(), item.getName())).collect(Collectors.toList()));
	}
	...
	public Optional<List<IdModel>> getAllFieldsOfController(String controllerId) {
		...
		return extractListOfItems(fields).map(
				e -> e.stream().map(item -> new IdModel(item.getID(), item.getName())).collect(Collectors.toList()));
	}
	...
	public Optional<ResultOfPartialResultOfHistoryRegion> getPartialHistoryOfField(Set<IdModel> fieldIds,
			String controllerId, XMLGregorianCalendar from, XMLGregorianCalendar to, int position)
		...
		ResultOfPartialResultOfHistoryRegion res = service.getFieldsPartialHistory2(token, from, to, arr, null, 1000,
				position);
